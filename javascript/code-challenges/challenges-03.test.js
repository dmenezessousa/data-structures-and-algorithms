...
const typeNum = (arr) => {
  ...
  return arr.filter((item) => {
    if (typeof item === "number") { // if the item is a number
      return item;
    }
  });
};
...
const containsAnd = (arr) => {
  // Solution code here...
  return arr.filter((item) => {
    if (item.includes("and")) { // if the item includes the word "and"
      return item;
    }
  });
};
...
const oddValues = (arr) => {
  // Solution code here...
  return arr.filter((item) => { // filter the array
    if (item % 2 !== 0) {
      return item;
    }
  });
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  return arr.filter((item) => {
    if (!forbiddenValues.includes(item)) { // if the item is not in the forbiddenValues array
      return item;
    }
  });
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter((item) => {
    if (item.baseStat > minBaseStat) { // if the baseStat is greater than the minBaseStat
      return item;
    }
  });
};
...
const getStatName = (arr, minBaseStat) => {
  ...
  }).map((item) => { // map the new array created by the .filter() method without the baseStats that are less than the minBaseStat
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  return arr.filter((item) => {
    if (!item.children) { // if the item does not have children
      return item;
    }
  });
};
...
const evenOddNumericValues = (arr) => {
  ...
  }).map((item) => { // map the new array created by the .filter() method without the non-numeric values
